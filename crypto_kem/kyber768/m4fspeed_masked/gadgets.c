...
void secb2a_modp(size_t nshares,
  ...
  masked_dense2bitslice(nshares - 1, BSSIZE, COEF_NBITS, zp_str, 1, nshares,
                        zp_dense, 1, nshares);
  ...
  masked_dense2bitslice(nshares - 1, BSSIZE, COEF_NBITS, in, in_msk_stride,
...
void masked_cbd(size_t nshares, size_t eta, size_t n_coeffs, size_t p,
  ...
  for (i = 0; i < eta; i++) {
    for (d = 0; d < nshares; d++) {
      ...
      sp[(i * 2) * nshares + nshares + d] =
          b[i * b_data_stride + d * b_msk_stride];
  ...
  secb2a_modp(nshares, p, z_str, 1, nshares);

  masked_bitslice2dense(nshares, n_coeffs, kbits, z, z_msk_stride,
                        z_data_stride, z_str, 1, nshares);
