 ...
 *accumulate in
 *              - polyvec *b:                 pointer to input vector of
 ...
 *is generated
 **************************************************/
static void matacc(poly *r, polyvec *b, unsigned char i,
                   const unsigned char *seed, int transposed) {
  ...
  for (int j = 0; j < KYBER_K; j++) {
    ...
    while (ctr < KYBER_N / 4) {
      ...
      if (val0 < KYBER_Q) {
        c[k++] = (int16_t)val0;
        if (k == 4) {
          doublebasemul_asm_acc(&r->coeffs[4 * ctr], &b->vec[j].coeffs[4 * ctr],
                                c, zetas[ctr]);
          ctr++;
          k = 0;
        }
 ...
 **************************************************/
unsigned char indcpa_enc_cmp(const unsigned char *c, const unsigned char *m,
...
indcpa_dec(unsigned char *m, const unsigned char *c, const unsigned char *sk) {
  poly mp, bp;
  poly *v = &bp;
