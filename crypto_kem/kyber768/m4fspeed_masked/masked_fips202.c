...
#define XORU64(value, address, byte) \
    do { (value)[(address) >> 3] ^= (((uint64_t) (byte)) << 8*((address) & 0x7)); } while (0)
#define ExtractU64(value, address) \
    (((value)[(address) >> 3] >> 8*((address) & 0x7)) & 0xFF)
void masked_hash_keccak(
...
void masked_shake256(
    ...
    masked_hash_keccak(
            output, outlen, out_msk_stride, out_data_stride,
            input, inlen, in_msk_stride, in_data_stride,
            136, 0x1f
            );
}
...
void masked_sha3_512(
    ...
    masked_hash_keccak(
            output, 64, out_msk_stride, out_data_stride,
            input, inlen, in_msk_stride, in_data_stride,
            72, 0x06
            );
}
