...
// unsigned char x, unsigned char y); void kyber_shake128_squeezeblocks(unsigned
// char *output, size_t nblocks, shake128ctx *s);
void masked_shake256_prf(unsigned char *output, size_t outlen,
                         const unsigned char *key, unsigned char nonce);
// void sha3_512_masked_check(uint8_t *output, const uint8_t *input, size_t
// inlen);

//#define hash_h(OUT, IN, INBYTES) sha3_256(OUT, IN, INBYTES)
#define masked_hash_g(OUT, IN, INBYTES) masked_sha3_512_check(OUT, IN, INBYTES)
...
#define masked_prf(OUT, OUTBYTES, KEY, NONCE)                                  \
  masked_shake256_prf(OUT, OUTBYTES, KEY, NONCE)
