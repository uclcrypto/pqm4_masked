...
void masked_poly_tomsg(unsigned char *msg, StrAPoly str_r) {
  ...
  for(int i=0; i < KYBER_INDCPA_MSGBYTES*(KYBER_MASKING_ORDER+1); ++i) msg[i] = 0;
   for(int i=0; i < KYBER_N/8; ++i){
      for(int j=0; j < 8; ++j){
        for(int k=0; k < KYBER_MASKING_ORDER+1; ++k) in.shares[k] = str_r[k][8*i+j];
        kyber_decryption(&in, &out);
        for(int k=0; k < KYBER_MASKING_ORDER+1; ++k) msg[i+k*(KYBER_INDCPA_MSGBYTES)] |= ((out.shares[k])&1)<<j; 
      }
    }
