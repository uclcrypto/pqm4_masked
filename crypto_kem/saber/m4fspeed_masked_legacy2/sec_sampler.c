...
#define MASKING_ORDER (NSHARES-1)
#define D NSHARES
...
void linear_refresh_masks(uint32_t* x, int q){
  uint16_t r;
...
void convert_B2A(uint32_t* x, uint32_t* y, unsigned k, unsigned q){
  ...
  for(int i=0; i < MASKING_ORDER+1; ++i){
    y[i] = (T[i] >> (16*(x[MASKING_ORDER])))&0x1FFF;
  }
...
void linear_arithmetic_refresh(uint32_t* x, unsigned q){
  int r;
  for(int i=0; i< MASKING_ORDER; ++i){

    r = (int) rand32()%q;     
    x[i] = (x[i] + r)%q;
    x[MASKING_ORDER] = (x[MASKING_ORDER] - r + q)%q;
  }
}
