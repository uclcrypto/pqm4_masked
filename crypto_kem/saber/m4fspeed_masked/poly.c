...
void MatrixVectorMulKeyPairNTT(uint8_t pk[SABER_INDCPA_PUBLICKEYBYTES],
...
uint32_t MatrixVectorMulEncNTT(uint8_t ct0[SABER_POLYVECCOMPRESSEDBYTES],
...
void InnerProdDecNTT(uint8_t m[SABER_KEYBYTES],
  ...
  for (i = 0; i < SABER_N; i++) {
    poly[i] =
        (poly[i] + h2 - (cm[i] << (SABER_EP - SABER_ET))) >> (SABER_EP - 1);
  }
