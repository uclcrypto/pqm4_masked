...
/* This function reduces its input mod p */
uint32_t POLp2BS_cmp(const uint8_t bytes[SABER_POLYCOMPRESSEDBYTES],
                     const uint16_t data[SABER_N]) {
  size_t j;
  const uint16_t *in = data;
...
void POLT2BS(uint8_t bytes[SABER_SCALEBYTES_KEM],
             const uint16_t data[SABER_N]) {
  size_t j;
  const uint16_t *in = data;
  uint8_t *out = bytes;
  ...
  for (j = 0; j < SABER_N / 8; j++) {
    ...
    out[1] = (uint8_t)(((in[2] >> 2) & 0x01) | ((in[3] & 0x7) << 1) |
                       ((in[4] & 0x7) << 4) | (in[5] << 7));
    out[2] =
        (uint8_t)(((in[5] >> 1) & 0x03) | ((in[6] & 0x7) << 2) | (in[7] << 5));
    in += 8;
    out += 3;
  }
...
/* This function reduces its input mod T */
uint32_t POLT2BS_cmp(const uint8_t bytes[SABER_SCALEBYTES_KEM],
                     const uint16_t data[SABER_N]) {
  size_t j;
  const uint16_t *in = data;
  const uint8_t *out = bytes;
  ...
  for (j = 0; j < SABER_N / 8; j++) {
    fail |=
        out[0] ^ (uint8_t)((in[0] & 0x7) | ((in[1] & 0x7) << 3) | (in[2] << 6));
    fail |= out[1] ^ (uint8_t)(((in[2] >> 2) & 0x01) | ((in[3] & 0x7) << 1) |
                               ((in[4] & 0x7) << 4) | (in[5] << 7));
    fail |= out[2] ^ (uint8_t)(((in[5] >> 1) & 0x03) | ((in[6] & 0x7) << 2) |
                               (in[7] << 5));
    in += 8;
    out += 3;
  }
...
uint32_t POLVECp2BS_cmp(const uint8_t bytes[SABER_POLYVECCOMPRESSEDBYTES],
                        const uint16_t data[SABER_L][SABER_N]) {
  size_t i;
  uint32_t fail = 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// The following functions are for compressed secret. Secrets are stored with
/// their 4-bit value in [-SABER_MU/2, SABER_MU/2]. ///
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
// void POLVECmu2BS(uint8_t bytes[SABER_INDCPA_SECRETKEYBYTES], const uint16_t
// data[SABER_L][SABER_N])
// {
//     size_t i;
//     for (i = 0; i < SABER_L; i++) {
...
// void BS2POLVECmu(const uint8_t bytes[SABER_INDCPA_SECRETKEYBYTES], uint16_t
// data[SABER_L][SABER_N])
// {
//     size_t i;
//     for (i = 0; i < SABER_L; i++) {
